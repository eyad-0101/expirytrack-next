                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      ...
                      {isAdmin && (item.userUsername || item.userEmail) && (
                        <p className="mt-1 flex items-center gap-1 text-[11px] text-ink-400 dark:text-ink-500">
                          <span>👤</span>
                          <span className="truncate">
                            {item.userUsername ? `@${item.userUsername}` : item.userEmail}
                          </span>
                        </p>
                      )}
                    </div>
                      ...
                      >
                        ...
                        {isAdmin && (
                          <td className="px-4 py-3 text-xs text-ink-500 dark:text-ink-400 max-w-[140px]">
                            <span className="truncate block" title={item.userUsername ? `@${item.userUsername}` : (item.userEmail ?? "")}>
                              {item.userUsername ? `@${item.userUsername}` : (item.userEmail ?? "—")}
                            </span>
                          </td>
                        )}
